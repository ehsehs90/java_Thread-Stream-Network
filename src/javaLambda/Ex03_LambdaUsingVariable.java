 ...
 * 클래스의 멤버 (필드 + 메소드)의 로컬 변수 (지역 변수)의 사용에 약간의 제약이 있음
 * 
 * 특히, this keyword를 사용할 때 주의해야 함
 * this : 현재 사용되는 객체의  reference
 * 람다식은 익명 객체를 만들어 내는 코드
 * this keyword를 쓰면 익명 객체를 지정하지 않음 -> 상위 객체를 지칭하게 됨
 * 
 *
...
class OuterClass {
	...
	class InnerClass {
		...
		Exam03_LambdaIF fieldLambda = () -> {
			...
			System.out.println("this.InnerClass의 객체를 찾아요 : " + this.innerField);
			System.out.println(this.getClass().getName());
		};
		Exam03_LambdaIF fieldLambda1 = new Exam03_LambdaIF() {
		...
		public void innerMethod() {
			...
			localLambda = () -> {
				...
				// 함수가 끝나면 사라지기 때문에 람다 객체 안에서 local 변수를 사용할 수 없음
				// localVal = 50;
			};
			localLambda.myFunc();
		}
