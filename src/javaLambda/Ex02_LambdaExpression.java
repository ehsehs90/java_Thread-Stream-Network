 ...
 * (인자1, 인자2, 인자3, ...)->{실행 코드}
 * 
 * 1. 람다식 (매개변수)의 이름은 개발자가 지정할 수 있음
 * 
 * 2. 매개변수의 타입은 컴파일러의 타입 유추에 의해서 알 수 있기 때문에
 * 	    람다식에서는 일반적으로 매개변수의 타입을 지정하지 않음
 * 
 * 3. 만약 매개변수가 1개인 경우 ()도 생략할 수 있음
 * 4. 만약 실행문이 1개인 경우 {}도 생략할 수 있음
 * 5. 매개변수가 없다면 ()는 생략할 수 없음
 * 6. 실행문에 당연히 return 구문이 들어올 수 있음
 * 
 * 7. 만약 실행문에 return 구문이 1개만 존재하면 {}와 return keyword도 생략 가능
 * 
 ...
 * 
 * 그래서 interface를 사용할 때 어노테이션을 이용해서 check 할 수 있음
 * @FunctionalInterface을 이용해서 
 * 해당 interface가 람다의 target type이 될 수 있는지
 * compiler에 의한 check를 해줌 ( 함수적 인터페이스 )
 * 
...
interface Exam02_LambdaIF {
	// interface의 method이므로 abstract 생략 가능
	abstract int myFunc(int a, int b);
}
public class Ex02_LambdaExpression {
	public static void main(String[] args) {
		Exam02_LambdaIF obj = (a, b) -> a + b;
		System.out.println(obj.myFunc(1, 2));
	}
}
